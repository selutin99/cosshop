<?php
	...
	if(isset($_GET['q']) && !empty($_GET['q'])){
		...
		$search = trim(urldecode(htmlspecialchars($_GET['cat'])));
		$res = returnName($search);
		$title = $res['name'];
	}
	else{
	...
	$num = 4;  
	$page = $_GET['page'];  
	$result = findCountProducts($search);  
...
<script>
    $(document).ready(function() {
        $(this).attr("title", "<?='Creative Online Store - '.$title;?>");
    });
</script>
		<!--FILTER-->
		<div class="container">
						...
						<div class="panel panel-default">
							...
							<div class="panel-body">
								...
								<div class="row">
									<?php			
										if(isset($_GET['q']) && !empty($_GET['q'])){
											$result = findProductsList($search, $start, $num);
										}
										else if(isset($_GET['cat']) && !empty($_GET['cat'])){
											$result = findProductsListByCategory($search, $start, $num);
										}
								...
								<div class="centered text-center">
									<?php  
										...
										if ($page != 1) $pervpage = '<a href="/search.php?q='.$search.'&page=1"><<</a><a href="/products.php?page='. ($page - 1).'"><</a> ';
										// Проверяем нужны ли стрелки вперед
										if ($page != $total) $nextpage = '  <a href="/search.php?q='.$search.'&page='. ($page + 1).'">></a><a href="/products.php?page='.$total.'">>></a> ';
										// Находим две ближайшие станицы с обоих краев, если они есть
										if($page - 2 > 0) $page2left = ' <a href="/search.php?q='.$search.'&page='. ($page - 2) .'">'. ($page - 2) .'</a>  ';
										if($page - 1 > 0) $page1left = '<a href="/search.php?q='.$search.'&page='. ($page - 1) .'">'. ($page - 1) .'</a>  ';
										if($page + 2 <= $total) $page2right = '  <a href="/search.php?q='.$search.'&page='. ($page + 2).'">'. ($page + 2) .'</a>';
										if($page + 1 <= $total) $page1right = '  <a href="/search.php?q='.$search.'&page='. ($page + 1).'">'. ($page + 1) .'</a>'; 
